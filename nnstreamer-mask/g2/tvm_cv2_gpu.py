...
lib = tvm.runtime.load_module("./512model_gpu.so")
target = "cuda"
dev = tvm.cuda()
...
#            break
    ...
    th = cv2.resize(cv2.threshold(np.squeeze(output_data), 0.9, 1, cv2.THRESH_BINARY)[-1], (height,width))
    #th = th[:,:,0]
    #print(frame.shape)
    img_in_rgb = frame
